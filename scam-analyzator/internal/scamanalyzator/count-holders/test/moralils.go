...
func main() {
	...
	chain := scamanalyzator.GetChainName(tokenData.Network)

	fmt.Printf("Проверяем адрес: %s в сети %s...\n", tokenData.TokensAddress, chain)

	url := fmt.Sprintf("https://deep-index.moralis.io/api/v2.2/erc20/%s/owners?chain=%s", tokenData.TokensAddress, chain)
